...
| **Phase 10 Step 5 — 感官距離 + optimizer 重接線**（距離排序、無 0–100 評分、移除 tds_factor/水質、label 殘餘清除、改名 ideal）| [`docs/PHASE10_STEP5_SCORING.md`](docs/PHASE10_STEP5_SCORING.md) |
| **Phase 10 Step 5.5 — Layer 2 全屬性重 fit**（6 軸→10 cotter 屬性、R² 閘門、互動內建於資料）| [`docs/PHASE10_STEP5_5_ALL_ATTRIBUTES.md`](docs/PHASE10_STEP5_5_ALL_ATTRIBUTES.md) |
| **Phase 10 Step 6 + Phase 11 — Feedback 迴圈設計**（配方產生機、三杯循環、ordinal 問卷、Claude 介入；**Step 6 + Phase 11 皆已實作 2026-05-22**）| [`docs/PHASE10_STEP6_FEEDBACK_LOOP.md`](docs/PHASE10_STEP6_FEEDBACK_LOOP.md) |
| **Phase 10 Step 6 — feedback schema 規格**（pairwise + ordinal、`compared_to`/`overall`/`attributes_vs`/`model_attributes_vs`/`absolute`、legacy 相容）| [`docs/FEEDBACK_FORMAT.md`](docs/FEEDBACK_FORMAT.md) |
| **Phase 11 — 迴圈引擎**（三杯循環狀態機、(1+λ) 擾動排程、digest、skip、flag 偵測、changelog、命名配方庫、單杯+跳過 UX；**已實作 2026-05-22**）| [`docs/PHASE11_LOOP_ENGINE.md`](docs/PHASE11_LOOP_ENGINE.md) |

> **🚧 進行中：Phase 10 — 感官重新奠基。** 模型從「6 化合物」改為「6 感官軸」。藍圖見 [`docs/PHASE10_SENSORY_REFOUNDING.md`](docs/PHASE10_SENSORY_REFOUNDING.md)（架構、訓練資料、8 步執行步驟）。舊化合物模型凍結於 git branch `compound-model-legacy`。**Step 1–5.5 完成（Step 5.5 於 2026-05-22）** — 6 感官軸定案 → **Step 5.5 改為 10 cotter CATA 屬性**（見下）+ Layer 2 風味模型 [`models/sensory.py`](models/sensory.py)（`predict_attributes()`,cotter 27-cell 回歸）+ **`label` 概念移除**（藍圖 §0,裁決 2026-05-21）→ [`data/ideal.json`](data/ideal.json) schema v6（per-roast 10 屬性 IDEAL,`light`/`medium_light`/`medium`/`moderately_dark`）+ **薄 Layer 1** [`models/layer1.py`](models/layer1.py)（平衡脫附式 `EY=E_MAX·(1−exp(−t/τ))·f_ratio`;`E_MAX` 由 Hoffman 單一**素浸泡**錨點解出、其餘 4 參數物理先驗;April/Champion 為技法沖煮、屬「不同黑箱」故剔除;**Step 5 後續移除 `BREWER_TAU_MULT` → Layer 1 brewer-agnostic、XL=standard 按水量縮放**;τ 為單錨點下 asserted DOF、絕對校準會抵銷、不值得再磨 —— 詳 `layer1.py` 內 NOTE）+ **Step 5 感官距離** [`models/distance.py`](models/distance.py)（`attribute_distance` —— 屬性 RMS 距離,**無 0–100 評分**,直接以「距目標多遠」排序;移除 `tds_factor`/化合物/label;TDS 影響已完全經屬性表達）+ optimizer 重接線（管線 `旋鈕→layer1→{tds,ey}→sensory→屬性→distance`;溫度為輸入、`dose×dial×steep` 為搜尋維度,distance 升冪排序;`optimize_parallel`/Channel A/B/`--label` 移除）+ **水質整組移除**（CLI flag/`resolve_water_profile`/`water_presets.py` 刪;Layer 1/2 本就不建模水質）+ 改名 `labels.json`→`ideal.json`、`labels.py`→`ideal.py`、`scoring.py`→`distance.py`。**Step 5.5（裁決 2026-05-22）：Layer 2 表徵從「6 軸（平均 cotter 屬性）」改成「10 個 cotter CATA 屬性各自回歸」** —— cotter 屬性值本就是消費者感受後的數字、跨屬性遮蔽已內建於資料,直接回歸即捕捉;全 17 屬性跑回歸、以 R²≥0.44 閘門保留 10 個（`Sour/Citrus/Tea.floral/Sweet/Cereal/Thick.viscous/Bitter/Astringent/Burnt/Dark.chocolate`）、砍 7 個（皆為保留屬性的弱雙胞或純雜訊）。6 軸降級成 `sensory.AXIS_VIEW` 顯示/問卷視圖。`distance` 改純未加權 RMS（閘門已濾雜訊）。**Step 6 完成（2026-05-22）：** feedback schema + webapp 重寫落地。**Step 6a** —— [`docs/FEEDBACK_FORMAT.md`](docs/FEEDBACK_FORMAT.md) 改 pairwise + ordinal schema（`compared_to`/`overall`/`attributes_vs`/`model_attributes_vs`/`absolute`,全 `>/=/<`,§4）、[`models/feedback.py`](models/feedback.py) 重寫（移除 `label`/`water` 必填、ordinal 驗證、`recompute_entry` 去 label/水質/T_ENV → 經 `score_logged_recipe` 重算 ey/tds/distance/attributes、legacy 純星等紀錄相容讀取）。**Step 6b** —— [`webapp.py`](webapp.py) 重寫（移除 label/水質/`optimize_parallel`、`/api/optimize` 改單一 `optimize(roast,brewer,temp,top)`、`_serialize_result` 改 10 屬性+distance）、[`templates/index.html`](templates/index.html) 移除 label 下拉+水質 UI+雷達 modal、加溫度控制（per-roast `DEFAULT_TEMP` 預設,切焙度自動帶入）、[`static/js/webapp.js`](static/js/webapp.js) 重寫（10 屬性 vs IDEAL bar、distance headline、§4 對照問卷:`compared_to` 選擇器+整體 `>/=/<`+逐屬性 7 群 `>/=/<`（模型預填）+`absolute`+comment+選填星等、logbook 改焙度過濾+成對比較摘要、極簡浸泡計時器）、新增 [`static/css/phase10.css`](static/css/phase10.css)（舊 `webapp.css` 不動,dead CSS 留置）。**Step 7 完成（2026-05-22）：** `diagnose_anchor.py` + `tests/` + `anchor_check.py` hook 重寫，過渡狀態結清。`diagnose_anchor.py` 改 Phase 10 兩層診斷（Layer 1 物理：Hoffman 素浸泡錨點重現 TDS 1.23、萃取單調;Layer 2 感官：ideal.json↔predict_attributes 自洽、optimizer 各焙度可達、距離鑑別 good≪over≪under、tim bracket 單調）—— 13 檢查、ASCII-only 輸出（避 cp950）、**以 exit code 回報**。`tests/` 全重寫成 6 檔（`test_layer1`/`test_sensory_distance`/`test_optimizer`/`test_feedback`/`test_output_and_cli`/`test_webapp`,**55 pytest PASS**）,舊 `test_compound_calibration`/`test_label_scoring`/`test_models` 刪。`.claude/hooks/anchor_check.py` 改：`TRIGGER_SUFFIXES` 換成 `layer1/sensory/distance/ideal.py`+`data/ideal.json`、**偵測改看 exit code**（修掉舊「grep `[ FAIL ]` 字串 → diagnose 一崩潰就注入假 PASS」的 bug —— 現在崩潰=非零 exit=block）。舊 `ey_model`/`tds_model`/`compounds`/`scoring.py`/`labels.py`/`water_presets.py` 已刪（保留於 branch `compound-model-legacy`）。**Phase 11 完成（2026-05-22）：迴圈引擎落地。** [`models/loop.py`](models/loop.py) —— per-roast (1+λ) 演化搜尋,三杯循環 `[實驗1,實驗2,冠軍重泡]` 狀態機;第 0 代冠軍由 optimizer Top-1 model-seed,每代提 2 個單旋鈕擾動（半徑照排程早大後細,溫度不搜）,三杯到齊後 `digest` 從成對 `overall` 选新冠軍（明確贏才換、平手守成）,勝者進下一循環;`skip_proposal` 同半徑重抽（後勤性、非味覺回饋）;`detect_flags` 掃 `feedback.jsonl` 找 `model_attributes_vs`↔`attributes_vs` 重複反向矛盾（≥2 次、`?` 排除）。第 3 杯固定為進來時的冠軍重泡（正統 (1+λ)、避開「digest 後最佳」的時序矛盾 —— 裁決見 [`docs/PHASE11_LOOP_ENGINE.md`](docs/PHASE11_LOOP_ENGINE.md) §3/§11）。`data/loop_state.json`（per-roast 狀態,lazy）+ [`data/refine_changelog.md`](data/refine_changelog.md)（Claude tier-3 模型改動紀錄）。加碼：[`models/saved.py`](models/saved.py) + `data/saved_recipes.json`（命名配方庫）。webapp 加模式頁籤（最佳化器 / 迴圈）、`/api/loop/*` + `/api/recipes` endpoints、feedback→loop 鉤子、單杯提案卡 + 跳過 + 重製鈕 + flag 通知。`tests/test_loop.py` ×20 → **75 pytest PASS**。**未採納**使用者「cotter 美味 TDS/EY 當目標」提議 —— cotter 喜好曲面近乎平、分兩群相反偏好,無客觀美味區（Step 4 §6 已查證；迴圈前提即「IDEAL 是使用者搜尋的移動靶」）。**Step 4 後續（2026-05-22）：medium_light IDEAL 已用使用者 ⭐5 杯重新校準（取代 Hoffman 文獻錨點 —— cotter hedonic 資料證實無「客觀最好」,單人系統以使用者實測 ⭐5 為準）;medium/moderately_dark placeholder 一併重算。****保留項：不同焙度偏好本身就不同,per-roast IDEAL 的 roast 維度靠 `models/sensory.py:_ROAST_OFFSET`（文獻先驗、未驗證）;`medium`/`moderately_dark` 目前是 placeholder,待 feedback 校準。** 下方「僅剩待辦 / 目前狀態」描述的是 Phase 10 之前的 Phase 8 狀態,Phase 10 落地後一併改寫。
