...
# axis. iso/leap both require ≥ LEAP_KNOB_DIFF_MIN knob diffs, so they never
# probe 1-knob neighbours by default. If the champion holds for several cycles
# (iso/leap can't improve), we burst-test the 6 single-knob neighbours one by
# one — exhaust the "diff by exactly 1 knob" hypothesis before declaring the
# champion a real local optimum. Manual single-knob override stays available
# for ad-hoc attribution; stall-trigger is the AUTOMATIC fallback when the
# loop can't find a better cup via multi-knob probes.
STALL_THRESHOLD = 3   # consecutive champion-wins before stall-trigger fires
STALL_KNOB_ROTATION = (
    ("dial", -1), ("dial", 1),
    ("steep_sec", -1), ("steep_sec", 1),
    ("dose", -1), ("dose", 1),
)
...
def _migrate(state: dict) -> dict:
    ...
    for loop in state.values():
        ...
        # stall-trigger counters added 2026-05-23; default to 0 for old loops
        loop.setdefault("stall_counter", 0)
        loop.setdefault("stall_rotation_idx", 0)
        for s in cycle.get("slots") or []:
            s.setdefault("stall", False)
    return state
...
def _apply_move(champion: dict, knob: str, sign: int, generation: int,
    ...
    Used by the default-cycle single-knob fallback (`_pick_experiment`) where
    perturbation shrinks generation-by-generation. For override + stall, use
    `_apply_single_step` instead — those are "test the immediate 1-knob
    neighbour" probes and must always be exactly one grid step.
...
def _apply_single_step(champion: dict, knob: str, sign: int, roast: str,
                       brewer_size: str, water_ml: float) -> dict | None:
    """Like _apply_move but ALWAYS one grid step (no generation-aware radius).
    Used by override + stall-trigger — both probe the *immediate 1-knob
    neighbour*, semantically a 1-step shift regardless of cycle generation.
    Returns None at grid edges (no-op)."""
    grid = _grid(knob, roast, brewer_size, water_ml)
    step = grid[2]
    base_snap = _snap(knob, champion[knob], grid)
    moved = _snap(knob, champion[knob] + sign * step, grid)
    if moved == base_snap:
        return None
    recipe = _knob_recipe(champion)
    recipe[knob] = moved
...
def _new_slot(role: str, recipe: dict, move, roast: str, brewer_size: str,
              temp: float, kind: str | None = None,
              stall: bool = False) -> dict:
    """Build a cycle slot. `kind` annotates how an experiment was generated:
    "single" = local single-knob perturbation, "iso" = iso-(TDS,EY) jump,
    "leap" = surrogate-guided multi-knob jump (Gold-Cup gated leap), None =
    champion re-brew slot. `stall` is True when this slot was generated by
    the stall-trigger (champion held for STALL_THRESHOLD cycles → burst-
    test 1-knob neighbours)."""
    return {
        "role": role,
        "recipe": recipe,
...
def override_to_single_knob(roast: str, recipe_id: str, knob: str,
    ...
    # User override = "I want to test ONE grid step on this knob" — single-step,
    # not generation-aware. The button label ("更細 ←") implies one step.
    new_recipe = _apply_single_step(
        loop["champion"], knob, sign,
...
def _build_cycle(loop: dict, cycle_index: int) -> dict:
    ...
    # exp1 — selection order:
    #   1. Stall-trigger: champion held for STALL_THRESHOLD cycles → burst-
    #      test 1-knob neighbours (covers the "true peak just 1 grid step off
    #      on a single axis" case that iso/leap structurally skip).
    #   2. Otherwise iso-(TDS,EY) jump: probe Layer 2's b_temp=0 + weak
    #      _GRIND_SLOPE assumption with a same-TDS/EY structural change.
    #   3. Fallback: single-knob via _pick_experiment when neither qualifies.
    exp1_recipe, exp1_move, exp1_kind, exp1_stall = None, None, None, False

    stall_count = loop.get("stall_counter", 0)
    stall_rot = loop.get("stall_rotation_idx", 0)
    if stall_count >= STALL_THRESHOLD and stall_rot < len(STALL_KNOB_ROTATION):
        # Advance through STALL_KNOB_ROTATION one direction per cycle until
        # we find a non-no-op move (skipping directions blocked at the grid
        # edge). Single-step (NOT generation-aware) — stall-trigger is
        # specifically the "1-knob neighbour" probe.
        while stall_rot < len(STALL_KNOB_ROTATION):
            knob, sign = STALL_KNOB_ROTATION[stall_rot]
            candidate = _apply_single_step(
                champion, knob, sign, roast, brewer, water_ml,
            )
            stall_rot += 1
            if candidate is not None:
                exp1_recipe = candidate
                exp1_move = (knob, sign)
                exp1_kind = "single"
                exp1_stall = True
                break
        loop["stall_rotation_idx"] = stall_rot

    if exp1_recipe is None:
        ...
        if iso_recipe is not None:
            ...
            if pick1 is None:   # champion boxed into a corner — degenerate
    ...
    slots.append(_new_slot("exp1", exp1_recipe, exp1_move, roast, brewer, temp,
                           kind=exp1_kind, stall=exp1_stall))
...
def _digest(loop: dict) -> dict:
    ...
    # Stall-trigger counters: any experiment winning RESETS rotation (the loop
    # found a better cup via iso/leap, no need to single-knob-probe); a
    # champion hold INCREMENTS the counter (iso/leap can't beat champion →
    # maybe the answer is a 1-knob neighbour we're skipping).
    if winner_role == "champion":
        loop["stall_counter"] = loop.get("stall_counter", 0) + 1
    else:
        loop["stall_counter"] = 0
        loop["stall_rotation_idx"] = 0
...
def start_loop(roast: str, brewer: str = "xl", temp: float | None = None) -> dict:
    ...
    loop = {
        ...
        "history": [],
        "stall_counter": 0,         # consecutive champion-wins (STALL_THRESHOLD trips it)
        "stall_rotation_idx": 0,    # next direction in STALL_KNOB_ROTATION to try
        "started_at": _now_iso(),
        "updated_at": _now_iso(),
    }
...
def current_proposal(roast: str) -> dict | None:
    ...
    return {
        **evaluated,
        ...
        "kind": slot.get("kind"),       # "single" / "iso" / "leap" / None
        "move": slot.get("move"),       # [knob, sign] or None — used by UI to label single-knob
        "stall": bool(slot.get("stall", False)),    # stall-trigger single-knob
        "cycle_index": loop["cycle"]["index"],
        "generation": loop["generation"],
        "skips": slot["skips"],
