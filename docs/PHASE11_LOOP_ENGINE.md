...
```
 ...
 optimizer.optimize ─seed─► champion        │
                                            ▼
        ┌── 三杯循環 [exp1, champion, exp2] ──┐
        │  exp1, exp2 = 冠軍的單旋鈕擾動        │   ← (1+λ=2)，擾動半徑照排程
        │  champion   = 進來時的冠軍，重泡(中間) │
        └───────────────┬─────────────────────┘
...
## 3. 三杯循環 `[exp1, champion, exp2]`

每個循環三杯，照順序泡：

1. **exp1** — 冠軍的單旋鈕擾動。
2. **champion** — 本循環**進來時**的冠軍，在中間重泡。
3. **exp2** — 另一個、不同旋鈕的冠軍擾動。

**為什麼冠軍夾在兩個實驗中間**（不是放最後）—— 這個順序讓**兩個實驗都跟冠軍直接
cup-adjacent**（記憶最新）：

```
cup 1 (exp1) ──記憶相鄰── cup 2 (champ) =>  exp1 vs champion 直接邊
cup 2 (champ) ──記憶相鄰── cup 3 (exp2) =>  exp2 vs champion 直接邊
```

兩條 experiment↔champion 邊在**每個循環**（含 cycle 1）都是 cup-adjacent 直接得來、
不需要轉移合成。**這是對藍圖 §3 原 `[exp1, exp2, champion]` 順序的修訂**（2026-05-23）：
舊順序下 cycle 1 的 `exp1↔冠軍` 邊得靠 `compose(exp1↔exp2, exp2↔冠軍)` 兩段合成
（方向相反就 ambiguous），且狀態機跟 cycle ≥2 行為不一致。新順序消掉這兩個問題。

**為什麼冠軍是進來時的冠軍**（不是 digest 後的贏家）—— 這是正統的 (1+λ)：親代
（冠軍）在整個世代內固定，選擇是世代之間的離散步驟。中間那杯冠軍重泡兼三件事:
重新錨定味覺記憶（藍圖 §3「重泡冠軍取代記住冠軍」）、給一杯能好好喝的、做絕對錨點
檢查。**digest 後的贏家成為下一循環的冠軍** —— 勝出的實驗會在下一循環當 cup 2
被重泡。藍圖 §3 字面另有「第 3 杯＝吃完回饋後算出的最佳」一句，但那會撞時序矛盾
（必須先有第 3 杯的回饋才能 digest），且讓狀態機分循環 1 / 循環 ≥2 兩套寫法。
正統 (1+λ) + 中間冠軍順序兩者一起 = 統一、可測、可逐杯獨立驗證。
...
新順序 `[exp1, champion, exp2]` 下，digest 只用兩條 cup-adjacent 直接边：

```
cup 2 的 overall (champion vs exp1)  -- invert -->  exp1 vs champion
cup 3 的 overall (exp2 vs champion)  ----------->   exp2 vs champion
```

每個循環、無條件直接取得、不需合成。

`_select_winner`：兩個實驗都明確贏 → 因為新順序內**沒有** cycle 內的 `exp1↔exp2`
邊（兩個實驗不 cup-adjacent），fallback 為**取 exp1**（確定性、保守、罕見情況）；
只有一個贏 → 該實驗；都沒明確贏 → 冠軍守成。

webapp 會把每杯問卷的 `compared_to` 預選成迴圈建議的「上一杯」，所以正常流程下
digest 拿到的都是乾淨的直接边；使用者改選別杯時，該边取不到就退化成冠軍守成。

cup 1 (exp1) 的 `compared_to` 預選為**上一個循環的最後一杯**（在新順序下 = 上一循環
的 exp2，儲存在 `loop.last_cup`） —— 這條跨循環邊不進當前循環的 digest，只作為使用者
味覺記憶的連續性、與自由欄文字 / `attributes_vs` 的對照背景。
...
|---|---|---|---|
| 三杯順序 | §3 字面 `[exp1, exp2, champion]` | `[exp1, champion, exp2]`（**2026-05-23 修訂**） | 冠軍夾在中間 → 兩個實驗都 cup-adjacent 直接得 exp↔champion 邊，每個循環一致（含 cycle 1） |
| 中間冠軍杯角色 | §3 字面「digest 後的最佳」 | 進來時的冠軍重泡；勝者進下一循環 | 正統 (1+λ)、無時序矛盾、狀態機統一 |
| digest 時機 | — | 三杯全到齊後 | 統一、可測 |
| exp1↔冠軍 边 | §3 鏈式 exp2↔exp1 | cup-adjacent 直接（cup 2 的 overall invert）—— 每個循環一致 | 直接边比轉移合成乾淨；新順序消掉 cycle 1 的合成 |
| exp1↔exp2 tiebreak | §3 cup-adjacent 直接 | 新順序下無 cycle 內直接邊；雙贏時 fallback 取 exp1 | 罕見情況；保守確定性 |
