...
| 3 | ✅ 重寫 `data/labels.json` —— IDEAL 改 6 感官軸（per-roast）→ [`PHASE10_STEP3_LABELS.md`](PHASE10_STEP3_LABELS.md) | `data/labels.json` |
| 4 | ✅ 薄 Layer 1（平衡脫附式 knob→TDS/EY）→ [`models/layer1.py`](../models/layer1.py)；5 參數,`E_MAX` 由 Hoffman 單一素浸泡錨點解出（April/Champion 技法沖煮、屬不同流程故剔除），其餘 4 個物理先驗,文件 [`PHASE10_STEP4_LAYER1.md`](PHASE10_STEP4_LAYER1.md)。舊 `ey_model`/`tds_model`/`compounds` 退役（Step 5 改接線後刪）| `models/layer1.py` |
