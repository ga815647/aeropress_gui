...
def _print_single(result: dict, roast_code: str, index: int) -> None:
    ...
    print(f"第 {index} 名  風味評分：{result['score']:.1f} / 100   "
          f"[label={label}]" + (f"  recipe_id={rid}" if rid else ""))
    print(
...
def print_terminal(results, roast_code: str, water_gh: float, water_kh: float) -> None:
    """Accepts either a flat list of results (single-label mode) or a
    dict[label] → list (Channel B parallel mode)."""
    if not results:
        print("無可用結果。")
        return
    ...
    if isinstance(results, dict):
        sample = next(iter(results.values()), [{}])[0] if results else {}
        brewer = sample.get("brewer", "—")
        print(f" 機型：{brewer}  |  烘焙度：{roast_name} ({roast_code})")
        print(f" 水質：GH {water_gh:g} ppm  /  KH {water_kh:g} ppm")
        print(f" 模式：Channel B — 各 label 各自最佳化 ({len(results)} labels)")
        print("════════════════════════════════════════════════════════\n")
        for label, items in results.items():
            print(f"─── label: {label} ─────────────────────────────────────")
            if not items:
                print("  (this label produced no candidates)\n")
                continue
    ...
    print(f" 水質：GH {water_gh:g} ppm  /  KH {water_kh:g} ppm")
    print(f" 模式：label = {results[0].get('label', 'balanced')}")
    print("════════════════════════════════════════════════════════\n")
